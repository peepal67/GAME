...
int main(void)
{
    ...
    if (wallTexture.id == 0 || doorTexture.id == 0 || floorTexture.id == 0 || ceilingTexture.id == 0) {
        CloseWindow();
        return 1;
    }
    ...
    roomA.AddPickup(new Pickup({2, 0.25f, 0}, keyItem, kKeyModelPath, 5.0f));
    ...
    notes.push_back(new Note({-4, 1.2f, -70}, "The old records show the year was 19__."));
    notes.push_back(new Note({4, 1.2f, -70},  "Third digit: count the candles on the altar. Six."));
